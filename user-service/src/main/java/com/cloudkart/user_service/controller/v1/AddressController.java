...
public class AddressController {
        ...
        @GetMapping
        public ResponseEntity<ResponseDto<List<AddressDto>>> getUserAddresses(
                        @PathVariable UUID userId) {
                List<AddressDto> addresses = addressService.getAddressByUserId(userId);
                ResponseDto<List<AddressDto>> response = new ResponseDto<>(HttpStatus.OK, addresses,
        ...
        @ApiResponses({@ApiResponse(responseCode = "200", description = "HTTP Status OK"),
                        ...
                        @ApiResponse(responseCode = "500",
                                        ...
                                        content = @Content(schema = @Schema(
                                                        implementation = ErrorResponseDto.class)))})
        @GetMapping("/default")
        public ResponseEntity<ResponseDto<AddressDto>> getDefaultUserAddress(
                        @PathVariable UUID userId) {
                AddressDto defaultAddress = addressService.getDefaultUserAddress(userId);
                ResponseDto<AddressDto> response = new ResponseDto<>(HttpStatus.OK, defaultAddress,
        ...
        public ResponseEntity<ResponseDto<Long>> countUserAddresses(@PathVariable UUID userId) {
                ...
                ResponseDto<Long> response = new ResponseDto<>(HttpStatus.OK, count,
        ...
        public ResponseEntity<ResponseDto<AddressDto>> createAddress(@PathVariable UUID userId,
                ...
                if (addressCount >= 4) {
                        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                                        .body(new ResponseDto<>(HttpStatus.BAD_REQUEST, null,
                                                        "User cannot have more than 4 addresses"));
                }

                addressReqDto.setUserId(userId);
                AddressDto createdAddress = addressService.createAddress(addressReqDto);
                ResponseDto<AddressDto> response = new ResponseDto<>(HttpStatus.CREATED,
                                createdAddress, "Address created successfully");
        ...
        public ResponseEntity<ResponseDto<AddressDto>> updateAddress(@PathVariable UUID userId,
                ...
                ResponseDto<AddressDto> response = new ResponseDto<>(HttpStatus.OK, updatedAddress,
